...
const build = module.exports = fileName => {
  ...
  const simSRC = path.join(__dirname, '..', 'src', 'sim');
  const simDST = path.join(buildpath, 'sim');
  const uthashSRC = path.join(__dirname, '..', 'deps', 'uthash', 'src');
  const uthashDST = path.join(buildpath, 'uthash');
  const project = {fileName, baseName, buildpath, sketchName, programName};

  return actions.mkdirp(simDST)
    ...
    .then(() => actions.copyTree(simSRC, simDST))
    .catch(err => exitError('Error : Failed copy simulator lib\n', err))
    .then(() => actions.copyTree(uthashSRC, uthashDST))
    .catch(err => exitError('Error : Failed copy uthash lib\n', err))
