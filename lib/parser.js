...
class LuacParser {
  ...
  readInstructions() {
    ...
    for (let index = 0; index < instructionsCount; ++index) {
      ...
      instruction.raw = `0x${rawBuf.toString('hex').toUpperCase()}`;
      instruction.opcode = raw & 0x3F;
      instruction.opcodeName = OPCODES_NAMES[instruction.opcode];
      instruction.a = (raw & 0x3FC0) >> 6;

      if (this.iABx.indexOf(instruction.opcode) >= 0 || this.iAsBx.indexOf(instruction.opcode) >= 0) {
  ...
  readPrototype(parent, index) {
    const proto = {};
    const {funcName, baseName} = this.readFuncName();

    if (!parent) {
      Object.assign(proto, {funcName, baseName});
    } else {
