...
class LuacParser {
  ...
  readUpvalues() {
    ...
    for (let index = 0; index < upvaluesCount; ++index) {
      this.proto.upvalues.push(this.readString());
    }
  }
...
class PrototypeParser extends LuacParser {
  ...
  readPrototypes() {
    const prototypesCount = this.readInt();

    this.proto.protos = [];

    for (let index = 0; index < prototypesCount; ++index) {
      const protoParser = new PrototypeParser(this.readBuf(this.buf.length - this.cursor));

      this.proto.protos.push(protoParser.parse());

      this.cursor += protoParser.cursor;
    }
  }
  ...
  parse() {
    ...
    this.proto.firstLine = this.readInt();
    this.proto.lastLine = this.readInt();

    this.proto.numparams = this.readByte();
    this.proto.isvarargs = this.readByte();
    this.proto.maxstacksize = this.readByte();
