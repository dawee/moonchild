const dumpInstructionBC = instruction =>  `b = ${instruction.b}, c = ${instruction.c}`;
const dumpInstructionBX = instruction =>  `bx = ${instruction.bx}`;
const dumpInstructionParams = instruction => (
 `a = ${instruction.a}, ${'b' in instruction ? dumpInstructionBC(instruction) : dumpInstructionBX(instruction)}`
);
...
${proto.instructions.map(
  instruction => `  {.raw = ${instruction.raw}, .flag = ${instruction.flag}}, // ${dumpInstructionComment(instruction)}`
).join('\n')}
};
