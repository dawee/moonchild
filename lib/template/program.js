...
`const moon_upvalue ${proto.baseName}_upvalues[] PROGMEM = {
...
const dumpPrototypeContent = proto => `
  ...
  .constants_addr = (PGMEM_ADDRESS) ${proto.constants.length > 0 ? `${proto.baseName}_constants` : 'NULL'},
  .prototypes_count = ${proto.protos.length},
  .prototypes_addr = (PGMEM_ADDRESS) ${proto.protos.length > 0 ? `${proto.baseName}_prototypes` : 'NULL'},
  .upvalues_count = ${proto.upvalues.length},
  .upvalues_addr = (PGMEM_ADDRESS) ${proto.upvalues.length > 0 ? `${proto.baseName}_upvalues` : 'NULL'},
`;
...
void moon_run_generated() {
  moon_arch_run((PGMEM_ADDRESS) &${project.proto.baseName}_prototype);
}
`;
