...
static const ${constant.typeStruct} ${proto.baseName}_constants_value_${i} PROGMEM = {
  .type = ${constant.type}, .nodes = 1, .string_addr = MOON_SRAM_ACCESSOR(${proto.baseName}_constants_value_${i}_string), .length = ${constant.val.length}
};
`;
...
const dumpConstArrayElement = (proto, constant, i) => {
  return `{.is_progmem = TRUE, .is_copy = FALSE, .value_addr = MOON_SRAM_ACCESSOR(&${constant.isStatic ? constant.staticAddr : `${proto.baseName}_constants_value_${i}`})},`;
}
...
const dumpPrototypeContent = proto => `
  ...
  .instructions_addr = MOON_PGM_ACCESSOR(${proto.baseName}_instructions),
  .constants_count = ${proto.constants.length},
  .constants_addr = MOON_PGM_ACCESSOR(${proto.constants.length > 0 ? `${proto.baseName}_constants` : 'NULL'}),
  .prototypes_count = ${proto.protos.length},
  .prototypes_addr = MOON_PGM_ACCESSOR(${proto.protos.length > 0 ? `${proto.baseName}_prototypes` : 'NULL'}),
  .upvalues_count = ${proto.upvalues.length},
  .upvalues_addr = MOON_PGM_ACCESSOR(${proto.upvalues.length > 0 ? `${proto.baseName}_upvalues` : 'NULL'}),
`;
